	...
	helpers.query(
		...
		function(error, queryData) {
			...
			async.each(queryData, function(opportunity, callback) {
				...
				var formattedCloseDate = moment(new Date(opportunity.close_date)).day(-6).format("MM/DD/YYYY")
				var formattedStartDate = moment(new Date(opportunity.start_date)).day(-6).format("MM/DD/YYYY")
